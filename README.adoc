...
|[[input_argocd_namespace]] <<input_argocd_namespace,argocd_namespace>>
|The namespace used by Argo CD where the Application and AppProject resources should be created.
|`string`
|n/a
|yes

|[[input_dependency_ids]] <<input_dependency_ids,dependency_ids>>
|The IDs of the other modules on which this module depends on.
|`map(string)`
|`{}`
|no
...
|===
|Name |Description
|[[output_id]] <<output_id,id>> |The ID to pass other modules in order to refer to this module as a dependency.
|===
// END_TF_TABLES
====
